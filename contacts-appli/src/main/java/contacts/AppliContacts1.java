...
public class AppliContacts1 extends Application {
	...
	public final void start(Stage stage) {
		...
		try {
	    	...
	    	try {
				IDaoCompte dao = context.getBean( IDaoCompte.class );
				sbMessage.append( "\n    Couche DAO     : " ).append( dao.getClass().getPackageName() );
			} catch (NoSuchBeanDefinitionException e) {
			}
	    	try {
		    	IServiceCompte service = context.getBean( IServiceCompte.class );
	    		sbMessage.append( "\n    Couche Service : " ).append( service.getClass().getPackageName() );
			} catch (NoSuchBeanDefinitionException e) {
			}
	    	try {
		    	IModelCompte model = context.getBean( IModelCompte.class );
	...
	public static class Main {
		public static void main(String[] args) {
			Application.launch( AppliContacts1.class, args);
		}
	}
